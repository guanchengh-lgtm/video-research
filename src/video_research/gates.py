...
def _failed(
    gate_id: str, name: str, code: DiagnosticCode, detail: str, interval: TimeInterval | None = None
) -> GateResult:
    return GateResult(gate_id, name, GateOutcome.FAIL, detail, Diagnostic(code, detail, interval))
...
def gate_transcript(
    transcript: Transcript, duration_ms: int, expected_language: str | None
) -> GateResult:
    ...
    if transcript.kind is TranscriptKind.NONE or not transcript.segments:
        return _failed(
            "G2", name, DiagnosticCode.NO_USABLE_TRANSCRIPT, "no transcript segments were produced"
        )
    ...
    if expected_language and transcript.language and transcript.language != expected_language:
        return _failed(
            "G2",
            name,
            DiagnosticCode.TRANSCRIPT_LANGUAGE_UNEXPECTED,
...
def gate_timeline_partition(coverage: CoverageManifest) -> GateResult:
    ...
    return _failed(
        "G3", "timeline partition", DiagnosticCode.TIMELINE_NOT_PARTITIONED, "; ".join(defects)
    )
        ...
        for ref in ledger.evidence:
            if not ref.span.interval.within(timeline):
                return _failed(
                    "G5",
                    name,
                    DiagnosticCode.EVIDENCE_OUT_OF_RANGE,
...
def gate_schema(schema_valid: bool, detail: str = "") -> GateResult:
    ...
    return _failed(
        "G7",
        "canonical schema",
        DiagnosticCode.CANONICAL_SCHEMA_INVALID,
...
def gate_unattended(manual_rescue: bool) -> GateResult:
    ...
    return _failed(
        "G9", "unattended", DiagnosticCode.MANUAL_RESCUE_USED, "a manual rescue was required"
    )
